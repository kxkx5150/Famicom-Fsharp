...
//   let a = (address &&& 0xFF00) lor (wrapping_add (address &&& 0xFF) 1) in
...
//   push_byte cpu ((value &&& 0xFF00) lsr 8);
...
// let and_op cpu args = cpu.a <- set_nz_flags cpu (cpu.a &&& args)
...
//   let result = set_nz_flags cpu ((args lsl 1) &&& 0xFF) in
//   write_target cpu target result
...
//   let carry = if cpu.carry then 1 else 0 in
//   cpu.carry <- args &&& 0x80 > 0;
//   let result = ((args lsl 1) lor carry) &&& 0xFF in
//   write_target cpu target (set_nz_flags cpu result)
...
//   cpu.carry <- sum > 0xFF;
//   (* Oh boy... *)
//   cpu.overflow <- (lnot (cpu.a lxor args)) &&& (cpu.a lxor sum) &&& 0x80 > 0;
